...
A validação cruzada ou \textit{Cross-Validation} é uma técnica para verificar o quão eficiente é a capacidade de generalização do modelo, ou seja, qual a precisão ao prever um novo conjunto de dados. Com a validação cruzada evita-se o \textit{overfitting},  A validação cruzada consiste em \cite{james2013introduction}:
...
\begin{enumerate}
    ...
    \item O conjunto de validação é utilizado para ajustar os parâmetros. Isto é, após o treino verifica-se a precisão do modelo e caso esteja baixa realiza-se os ajustes necessários no algoritmo;
    \item Ao final verifica-se a generalização do algoritmo treinado, utilizando o conjunto de teste.
\end{enumerate}

A validação cruzada possue ainda algumas variações, sendo elas \cite{james2013introduction}:
\begin{enumerate}
    \item \textit{k-Fold Cross-Validation};
    \item \textit{Leave-one-out cross-validation (LOOCV)};
    \item \textit{holdout cross-validation};
\end{enumerate}
